...
TEST(RentItNowTest_CAR_REPOSITORY, SaveUser)
{
    ...
    data["ABCD123456"]=std::make_shared<User>(testUser);
    EXPECT_TRUE(UsersRepository::getInstance().save(data));

}
TEST(UsersRepositoryTest, SaveAndLoadUser) {
    // Create a test user
    User user( "John", "Doe", "123 Main St", "1234-5678-9012-3456", "ABCD123456");

    // Save the user to the repository
    std::map<std::string, std::shared_ptr<User>> data;
    data[user.getDrivingLicense()] = std::make_shared<User>(user);
    EXPECT_TRUE(UsersRepository::getInstance().save(data));
    ...
    auto loadedUser = loadedData[user.getDrivingLicense()];
